...
import uk.co.creative74.springmvchibernate.controller.AppController;
...
public class AppControllerTest {
	...
	public void listEmployees(){
		...
		Assert.assertEquals(appController.listEmployees(model), "demo/allemployees");
	...
	public void newEmployee(){
		Assert.assertEquals(appController.newEmployee(model), "demo/registration");
		Assert.assertNotNull(model.get("employee"));
		Assert.assertTrue((Boolean)model.get("create"));
	...
	public void saveEmployeeWithValidationError(){
		...
		Assert.assertEquals(appController.saveEmployee(employees.get(0), result, model), "demo/registration");
	}
	...
	public void saveEmployeeWithSuccess(){
		...
		doNothing().when(service).saveEmployee(any(Employee.class));
		Assert.assertEquals(appController.saveEmployee(employees.get(0), result, model), "demo/success");
	...
	public void editEmployee(){
		...
		Assert.assertEquals(appController.editEmployee(anyString(), model), "demo/registration");
		Assert.assertNotNull(model.get("employee"));
		Assert.assertFalse((Boolean)model.get("create"));
		Assert.assertEquals(((Employee)model.get("employee")).getId(), 0);
	}
	...
	public void updateEmployeeWithSuccess(){
		...
		when(service.isEmployeeSsnUnique(anyInt(), anyString())).thenReturn(true);
		doNothing().when(service).updateEmployee(any(Employee.class));
		Assert.assertEquals(appController.saveEmployee(employees.get(1), result, model), "demo/success");
		Assert.assertEquals(model.get("success"), "Employee Jeremy updated successfully");
	}
